...
export default function Mint() {
  const [uploadedImages, setUploadedImages] = useState({}); // used for debugging purposes, uncomment lines: 197 - 204
  ...
  const getCollections = async () => {
    ...
    try {
      ...
      for (let i = 0; i <= numberOfCollections; i++) {
        ...
        if (address != 0) {
          const collection = new ethers.Contract(address, nftABI, signer);
  ...
  const updateNft = (event) => {
    if (signer) {
      setContractNFT(
        new ethers.Contract(addresses.get(event.target.value), nftABI, signer)
      );
    }
  };
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      const tx2 = await contractMarketplace.addItem(
        contractNFT.address,
        tokenId
      );
      await tx2.wait();
      ...
      if (!isMarketplaceApproved) {
        const tx3 = await contractNFT.setApprovalForAll(
    ...
    <div>
      {ipfs ? (
        <div className="container">
          <h5 className={styles.title}> Mint </h5>
          <form
